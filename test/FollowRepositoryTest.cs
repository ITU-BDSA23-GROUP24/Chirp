...
public class FollowRepositoryTest
{
    ...
    /// <summary>
    /// The setup for cheep repo-test, here the connection to the database and the dbContext is created 
    /// </summary>
    ...
    {
        var a1 = new Author()
            { AuthorId = 1, Name = "existingAuthor", Cheeps = new List<Cheep>() };
        var a2 = new Author()
            { AuthorId = 2, Name = "Luanna Muro", Cheeps = new List<Cheep>() };
        var a3 = new Author()
            { AuthorId = 3, Name = "Filip Muro", Cheeps = new List<Cheep>() };
    ...
    /// </summary>
    [Fact]
    public async void AddFollow()
    {
        //arrange
        string follower = "Filip Muro";
        string following = "existingAuthor";
        FollowRepository followRepository = new FollowRepository(_context);
        int size;
        
        //act
        size = _context.Follows.Count();
        await followRepository.AddFollower(follower, following);

        //assert
        Assert.Equal(size + 1, _context.Follows.Count());

        Assert.NotNull(_context.Follows
    ...
    public async void AddFollowWhileOneOrBothParametersDoesNotExist(string follower, string following)
    {
        ...
        Assert.Equal(size, _context.Follows.Count());
    }
    ...
    /// </summary>
    [Fact]
    public async void RemoveFollow()
    {
        //arrange
        string follower = "Filip Muro";
        string following = "Luanna Muro";
        FollowRepository followRepository = new FollowRepository(_context);
        int size;
